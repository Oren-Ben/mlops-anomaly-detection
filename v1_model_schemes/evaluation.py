...
class ModelEvaluation:
    ...
    def calc_metrics(self):
        for model_name in self.eval_config.keys():
            accuracy, cm, mar, far, f1, fpr, tpr, thresholds, roc_auc = (
                calc_model_metrics(
    ...
    def plot_metrics(self, title="Model Evaluation Metrics"):
        ...
        for i, model_name in enumerate(self.eval_config.keys()):
            ...
            ConfusionMatrixDisplay(
                confusion_matrix=self.eval_config[model_name]["cm"]
            ).plot(ax=cm_ax, cmap="Blues", values_format="d")
...
class ModelSelector:
    ...
    def __init__(
        ...
        model: AbstractFullModelPipeline,
        min_splits=1,
        max_splits=20,
    ) -> None:
    ...
    def run_model_permutations(self):
        ...
        for i in range(self.min_splits, self.max_splits + 1):
            ...
            accuracy, cm, mar, far, f1, fpr, tpr, thresholds, roc_auc = (
                calc_model_metrics(y_true=y_test, y_pred=y_pred, th=config["th"])
            )
